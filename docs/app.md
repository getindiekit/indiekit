<!-- Generated by documentation.js. Update this documentation by updating the source code. -->

### Table of Contents

-   [admin][1]
    -   [Parameters][2]
-   [auth][3]
    -   [indieauth][4]
        -   [Parameters][5]
    -   [scope][6]
        -   [Parameters][7]
-   [cache][8]
    -   [update][9]
        -   [Parameters][10]
    -   [delete][11]
        -   [Parameters][12]
    -   [read][13]
        -   [Parameters][14]
-   [microformats][15]
    -   [derviveContent][16]
        -   [Parameters][17]
    -   [derivePhoto][18]
        -   [Parameters][19]
    -   [derivePostType][20]
        -   [Parameters][21]
    -   [dervivePublishedProperty][22]
        -   [Parameters][23]
    -   [derviveSlug][24]
        -   [Parameters][25]
    -   [formEncodedToMf2][26]
        -   [Parameters][27]
    -   [htmlToMf2][28]
        -   [Parameters][29]
    -   [urlToMf2][30]
        -   [Parameters][31]
-   [micropub][32]
    -   [createMedia][33]
        -   [Parameters][34]
    -   [createPost][35]
        -   [Parameters][36]
    -   [deletePost][37]
        -   [Parameters][38]
    -   [query][39]
        -   [Parameters][40]
    -   [saveFile][41]
        -   [Parameters][42]
    -   [savePost][43]
        -   [Parameters][44]
    -   [undeletePost][45]
        -   [Parameters][46]
    -   [updatePost][47]
        -   [Parameters][48]
    -   [getPostTypes][49]
        -   [Parameters][50]
    -   [response][51]
        -   [Parameters][52]
-   [publication][53]
-   [publication][54]
    -   [Parameters][55]
-   [record][56]
    -   [create][57]
        -   [Parameters][58]
    -   [read][59]
        -   [Parameters][60]
-   [render][61]
    -   [Parameters][62]
-   [store/github][63]
    -   [getContents][64]
        -   [Parameters][65]
    -   [createFile][66]
        -   [Parameters][67]
    -   [updateFile][68]
        -   [Parameters][69]
    -   [deleteFile][70]
        -   [Parameters][71]
-   [store][72]
-   [utils][73]
    -   [createRandomString][74]
        -   [Examples][75]
    -   [deriveFileProperties][76]
        -   [Parameters][77]
        -   [Examples][78]
    -   [deriveMediaType][79]
        -   [Parameters][80]
        -   [Examples][81]
    -   [decodeFormEncodedString][82]
        -   [Parameters][83]
        -   [Examples][84]
    -   [error][85]
        -   [Parameters][86]
    -   [excerptString][87]
        -   [Parameters][88]
        -   [Examples][89]
    -   [normalizePath][90]
        -   [Parameters][91]
        -   [Examples][92]
-   [routes][93]

## admin

Responds to POST requests

### Parameters

-   `request` **[Object][94]** Express request object
-   `response` **[Object][94]** Express response object
-   `next` **[Function][95]** Express next function

Returns **[Object][94]** Express response object

## auth

Verify access tokens and determine scope.

### indieauth

Verifies that a token provides permissions to post to configured publication,
using [IndieAuth][96] to ensure only
authenticated users can use endpoint for posting to configured destination.

#### Parameters

-   `request` **[Object][94]** Express request object
-   `response` **[Object][94]** Express response object
-   `next` **[Function][95]** Express next function

Returns **[Function][95]** Call next middleware function

Returns **[Object][94]** Error response

### scope

Checks if scope in authenticated token contains specified scope.
Automatically handles `post` and `create` as the same thing

#### Parameters

-   `requiredScope` **[String][97]** Scope to check
-   `request` **[Object][94]** Express request object
-   `response` **[Object][94]** Express response object
-   `next` **[Function][95]** Express next function

Returns **[Function][95]** Call next middleware function

Returns **[Object][94]** Error response

## cache

Caches files to prevent application from making excessive requests to APIs

### update

Creates a new file in the cache, populating it with specified data

#### Parameters

-   `filePath` **[Object][94]** Location to cache file
-   `fileData` **[Object][94]** Data to write to file

### delete

Deletes the cache directory

#### Parameters

-   `dir` **[Object][94]** Location of cache directory

### read

Reads a file in the cache, fetching from store if not found

#### Parameters

-   `storePath` **[String][97]** Path to file in store
-   `cachePath` **[String][97]** Path to file in cache

Returns **([Promise][98] \| [Object][94])** Fetched file object

## microformats

Discover, parse and transform microformats2 objects

### derviveContent

Derives content (HTML, else object value, else property value)

#### Parameters

-   `mf2` **[Object][94]** microformats2 object

Returns **[Array][99]** Content

### derivePhoto

Combines referenced and attached photos into one object which can be used in
a microformats2 object. Attached photos are uploaded to GitHub.

#### Parameters

-   `mf2` **[Object][94]** microformats2 object
-   `files` **[String][97]** Photos attached as multipart data
-   `typeConfig` **[Object][94]** Post type configuration

Returns **[Promise][98]** Array of photo obejcts

### derivePostType

Combines referenced and attached photos into one object which can be used in
a microformats2 object. Attached photos are uploaded to GitHub.

#### Parameters

-   `mf2` **[Object][94]** microformats2 object

Returns **[String][97]** Post type

### dervivePublishedProperty

Derives published date (based on microformats2 data, else the current date)

#### Parameters

-   `mf2` **[Object][94]** microformats2 object

Returns **[Array][99]** Array containing ISO formatted date

### derviveSlug

Derives slug (using `mp-slug` value, slugified name else a random number)

#### Parameters

-   `mf2` **[Object][94]** microformats2 object
-   `separator` **[String][97]** Slug separator

Returns **[Array][99]** Array containing slug value

### formEncodedToMf2

Parses microformats in form-encoded POST request.
Adapted from [node-micropub-express][100]
by [Pelle Wessman][101]

#### Parameters

-   `body` **[String][97]** Form-encoded body

Returns **[Object][94]** mf2 object

**Meta**

-   **copyright**: (c) 2016, Pelle Wessman


### htmlToMf2

Parses microformats on HTML page

#### Parameters

-   `html` **[String][97]** HTML marked up with microformats
-   `properties` **[String][97]** mf2 properties to return

Returns **[Promise][98]** mf2 object

### urlToMf2

Parses microformats at a given URL

#### Parameters

-   `url` **[String][97]** URL path to post
-   `properties` **[String][97]** mf2 properties to return

Returns **[Promise][98]** mf2 object

## micropub

Process and transform microformats in request so that it can be published to
a destination.

### createMedia

Creates a new media file

#### Parameters

-   `request` **[Object][94]** Express request object
-   `response` **[Object][94]** Express response object

Returns **[Promise][98]** Express response object

### createPost

Creates a post

#### Parameters

-   `request` **[Object][94]** Express request object
-   `response` **[Object][94]** Express response object

Returns **[Promise][98]** Express response object

### deletePost

Deletes a post

#### Parameters

-   `request` **[Object][94]** Express request object
-   `response` **[Object][94]** Express response object
-   `next` **[Function][95]** Express next function

Returns **[Promise][98]** Express response object

### query

Returns an object containing information about this application

#### Parameters

-   `request` **[Object][94]** Express request object
-   `response` **[Object][94]** Express response object

Returns **[Promise][98]** Express response object

### saveFile

Saves a file

#### Parameters

-   `file` **[Object][94]** File object

Returns **[String][97]** Location of created post

### savePost

Creates a new post

#### Parameters

-   `mf2` **[String][97]** Microformats2 object
-   `files` **[String][97]** File attachments

Returns **[String][97]** Location of created post

### undeletePost

Updates a post

#### Parameters

-   `request` **[Object][94]** Express request object
-   `response` **[Object][94]** Express response object
-   `next` **[Function][95]** Express next function

Returns **[Object][94]** Express response object

### updatePost

Updates a post

#### Parameters

-   `request` **[Object][94]** Express request object
-   `response` **[Object][94]** Express response object
-   `next` **[Function][95]** Express next function

Returns **[Object][94]** Express response object

### getPostTypes

Returns an array of supported post types

#### Parameters

-   `pub` **[String][97]** Publication configuration

Returns **[Array][99]** Array of post types

### response

Returns success information

#### Parameters

-   `id` **[String][97]** Identifier
-   `location` **[String][97]** Location of post

Returns **[Object][94]** Success object

## publication

Get publication configuration

## publication

Gets a publication’s configuration and combines it with default values set by
application.

### Parameters

-   `configPath` **[Object][94]** Path to publication config

Returns **[Promise][98]** Configuration object

## record

Record of an action and associated changes, stored for later retrieval,
i.e. for updating or undelete a post.

### create

Creates a new record

#### Parameters

-   `url` **[Object][94]** URL activity relates to
-   `data` **[Object][94]** Activity data to record

### read

Reads a record

#### Parameters

-   `url` **[Object][94]** URL of post

Returns **[Object][94]** Memo

## render

-   **See: [Nunjucks API: renderString][102]**

Render a Nunjucks template string using context data

### Parameters

-   `string` **[String][97]** Template string
-   `context` **[String][97]** Context data

Returns **[String][97]** Rendered string

## store/github

Get, create and delete data at a specified path at configured GitHub repo.

### getContents

-   **See: [GitHub REST API v3: Get Contents][103]**

Reads content of a file or directory in a repository

#### Parameters

-   `path` **[String][97]** Path to file

Returns **[Promise][98]** GitHub HTTP response

### createFile

-   **See: [GitHub REST API v3: Create a file][104]**

Creates a new file in a GitHub repository

#### Parameters

-   `path` **[String][97]** Path to file
-   `content` **[String][97]** File content
-   `options` **[String][97]** Options

Returns **[Promise][98]** GitHub HTTP response

### updateFile

-   **See: [GitHub REST API v3: Update a file][105]**

Updates a file in a GitHub repository

#### Parameters

-   `path` **[String][97]** Path to file
-   `content` **[String][97]** File content
-   `options` **[String][97]** Options

Returns **[Promise][98]** GitHub HTTP response

### deleteFile

-   **See: [GitHub REST API v3: Delete a file][106]**

Deletes a file in a GitHub repository

#### Parameters

-   `path` **[String][97]** Path to file
-   `options` **[String][97]** Options

Returns **[Promise][98]** GitHub HTTP response

## store

Interfaces to remote locations where files and configuration are stored.

## utils

Common utility functions.

### createRandomString

Generates random alpha-numeric string, 5 characters long

#### Examples

```javascript
createRandomString() => 'b3dog'
```

Returns **[Object][94]** Alpha-numeric string

### deriveFileProperties

Derives additional file name properties

#### Parameters

-   `file` **[Object][94]** Original file object

#### Examples

```javascript
deriveFileProperties('brighton-pier.jpg') => {
  originalname: 'brighton-pier.jpg',
  filedate: '2019-03-03T05:07:09+00:00',
  filename: 'ds48s',
  fileext: '.jpg'
}
```

Returns **[Object][94]** File properties

### deriveMediaType

Derives media type and returns equivalent IndieWeb post type

#### Parameters

-   `file` **[Object][94]** Original file object

#### Examples

```javascript
deriveMediaType('brighton-pier.jpg') => 'photo'
```

Returns **[String][97]** Returns either 'photo', 'video' or audio

### decodeFormEncodedString

Decodes form-encoded string

#### Parameters

-   `str` **[String][97]** String to decode

#### Examples

```javascript
decodeFormEncodedString('foo+bar') => 'foo bar'
```

```javascript
decodeFormEncodedString('http%3A%2F%2Ffoo.bar') => 'http://foo.bar'
```

Returns **[String][97]** Decoded string

### error

Returns error information

#### Parameters

-   `id` **[String][97]** Identifier
-   `desc` **[String][97]** Description

Returns **[Object][94]** Error object

### excerptString

Gets first n words from a string

#### Parameters

-   `str` **[String][97]** String to excerpt
-   `n` **[Number][107]** Max number of words

#### Examples

```javascript
excerptString('Foo bar baz', 2) => 'Foo bar'
```

Returns **[String][97]** Excerpt

### normalizePath

Removes ‘/’ from beginning and end of string. Useful for constructing paths

#### Parameters

-   `str` **[Object][94]** String

#### Examples

```javascript
normalizePath('/foo/bar/') => 'foo/bar'
```

Returns **[Object][94]** Normalized object

## routes

[1]: #admin

[2]: #parameters

[3]: #auth

[4]: #indieauth

[5]: #parameters-1

[6]: #scope

[7]: #parameters-2

[8]: #cache

[9]: #update

[10]: #parameters-3

[11]: #delete

[12]: #parameters-4

[13]: #read

[14]: #parameters-5

[15]: #microformats

[16]: #dervivecontent

[17]: #parameters-6

[18]: #derivephoto

[19]: #parameters-7

[20]: #deriveposttype

[21]: #parameters-8

[22]: #dervivepublishedproperty

[23]: #parameters-9

[24]: #derviveslug

[25]: #parameters-10

[26]: #formencodedtomf2

[27]: #parameters-11

[28]: #htmltomf2

[29]: #parameters-12

[30]: #urltomf2

[31]: #parameters-13

[32]: #micropub

[33]: #createmedia

[34]: #parameters-14

[35]: #createpost

[36]: #parameters-15

[37]: #deletepost

[38]: #parameters-16

[39]: #query

[40]: #parameters-17

[41]: #savefile

[42]: #parameters-18

[43]: #savepost

[44]: #parameters-19

[45]: #undeletepost

[46]: #parameters-20

[47]: #updatepost

[48]: #parameters-21

[49]: #getposttypes

[50]: #parameters-22

[51]: #response

[52]: #parameters-23

[53]: #publication

[54]: #publication-1

[55]: #parameters-24

[56]: #record

[57]: #create

[58]: #parameters-25

[59]: #read-1

[60]: #parameters-26

[61]: #render

[62]: #parameters-27

[63]: #storegithub

[64]: #getcontents

[65]: #parameters-28

[66]: #createfile

[67]: #parameters-29

[68]: #updatefile

[69]: #parameters-30

[70]: #deletefile

[71]: #parameters-31

[72]: #store

[73]: #utils

[74]: #createrandomstring

[75]: #examples

[76]: #derivefileproperties

[77]: #parameters-32

[78]: #examples-1

[79]: #derivemediatype

[80]: #parameters-33

[81]: #examples-2

[82]: #decodeformencodedstring

[83]: #parameters-34

[84]: #examples-3

[85]: #error

[86]: #parameters-35

[87]: #excerptstring

[88]: #parameters-36

[89]: #examples-4

[90]: #normalizepath

[91]: #parameters-37

[92]: #examples-5

[93]: #routes

[94]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Object

[95]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Statements/function

[96]: https://www.w3.org/TR/indieauth/

[97]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/String

[98]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Promise

[99]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Array

[100]: https://github.com/voxpelli/node-micropub-express

[101]: https://kodfabrik.se

[102]: https://mozilla.github.io/nunjucks/api.html#renderstring

[103]: https://developer.github.com/v3/repos/contents/#get-contents

[104]: https://developer.github.com/v3/repos/contents/#create-a-file

[105]: https://developer.github.com/v3/repos/contents/#update-a-file

[106]: https://developer.github.com/v3/repos/contents/#delete-a-file

[107]: https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Number
