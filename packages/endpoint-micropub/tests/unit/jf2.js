...
test('Gets audio property (from string)', t => {
  const properties = JSON.parse(getFixture('jf2/audio-provided-string-value.jf2'));
...
test('Gets photo property (from string)', t => {
  const properties = JSON.parse(getFixture('jf2/photo-provided-string-value.jf2'));
...
test('Gets normalised photo property, adding provided text alternatives', t => {
  const properties = JSON.parse(getFixture('jf2/photo-provided-mp-photo-alt.jf2'));
...
test('Gets video property (from string)', t => {
  const properties = JSON.parse(getFixture('jf2/video-provided-string-value.jf2'));
...
test('Gets date from `published` property (short date)', t => {
  const properties = JSON.parse(getFixture('jf2/note-published-provided-short.jf2'));
...
test('Derives slug from unslugified `mp-slug` property', t => {
  const properties = JSON.parse(getFixture('jf2/note-slug-provided-unslugified.jf2'));
...
test('Derives slug, ignoring empty `mp-slug` property', t => {
  const properties = JSON.parse(getFixture('jf2/article-slug-provided-empty.jf2'));
...
test('Derives slug by generating random number', t => {
  const properties = JSON.parse(getFixture('jf2/note-slug-missing-no-name.jf2'));
