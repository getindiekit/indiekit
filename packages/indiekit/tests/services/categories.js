...
test('Returns array of available categories', async t => {
  const result = await getCategories(mockClient, t.context.categoriesWithArray);
  t.deepEqual(result, ['foo', 'bar']);
});

test('Fetches array from remote JSON file specified in `url` value', async t => {
  const scope = t.context.nock.reply(200, ['foo', 'bar']);
  const result = await getCategories(mockClient, t.context.categoriesWithUrl);
  t.deepEqual(result, ['foo', 'bar']);
  scope.done();
});
...
test('Returns empty array if remote JSON file not found', async t => {
  ...
  const error = await t.throwsAsync(getCategories(mockClient, t.context.categoriesWithUrl));
