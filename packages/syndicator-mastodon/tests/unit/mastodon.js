...
test('Posts a favourite', async t => {
  ...
  const result = await mastodon(t.context.options).postFavourite(t.context.tootUrl);
...
test('Throws error posting a favourite', async t => {
  ...
  await t.throwsAsync(mastodon(t.context.options).postFavourite(t.context.tootUrl), {
...
test('Posts a reblog', async t => {
  ...
  const result = await mastodon(t.context.options).postReblog(t.context.tootUrl);
...
test('Throws error posting a reblog', async t => {
  ...
  await t.throwsAsync(mastodon(t.context.options).postReblog(t.context.tootUrl), {
    message: 'Request failed with status code 404',
  });
});
...
test('Posts a status', async t => {
  ...
  const result = await mastodon(t.context.options).postStatus(t.context.status, t.context.options.url);
...
test('Throws error posting a status', async t => {
  ...
  await t.throwsAsync(mastodon(t.context.options).postStatus(t.context.status, t.context.options.url), {
    message: 'Request failed with status code 404',
  });
});
...
test.serial('Throws error fetching media to upload', async t => {
  ...
  await t.throwsAsync(mastodon(t.context.options).uploadMedia(t.context.media, t.context.publication), {
    message: /Not found/,
  });
});
...
test.failing('Uploads media and returns a media id', async t => {
  ...
  const result = await mastodon(t.context.options).uploadMedia(t.context.media, t.context.publication);
...
test('Returns false passing an object to media upload function', async t => {
  const result = await mastodon(t.context.options).uploadMedia({foo: 'bar'}, t.context.publication);
...
// Fails as Nock doesn’t send _httpMessage.path value used by form-data module
test.failing('Posts a status to Mastodon server with 4 out of 5 photos', async t => {
...
test('Throws an error posting a status to Mastodon server with 4 out of 5 photos', async t => {
  ...
  await t.throwsAsync(mastodon(t.context.options).post({
    content: {
      html: '<p>Here’s the cheese sandwiches I ate.</p>',
    },
    photo: [
