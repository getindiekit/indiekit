...
test('Creates a status with HTML content and doesn’t append Mastodon link', t => {
  const result = createStatus(
    JSON.parse(getFixture('jf2/note-content-provided-html-with-mastodon-link.jf2')),
    'https://mastodon.example',
  );
...
test('Creates a reblog with status URL and post content', t => {
  ...
  t.is(result.status, `Someone else who likes cheese sandwiches. ${t.context.tootUrl}`);
});
...
test('Creates a status with a photo', t => {
  ...
  }, 'https://mastodon.example', ['1', '2', '3', '4']);
...
test('Tests if string is a toot permalink', t => {
  t.true(isTootUrl('https://mastodon.example/@username/1234567890987654321', 'https://mastodon.example'));
...
test('Gets status ID from Mastodon permalink', t => {
  const result = getStatusIdFromUrl('https://mastodon.example/@username/1234567890987654321');

  t.is(result, '1234567890987654321');
});

test('Convert HTML to status text', t => {
  const result = htmlToStatusText('<p>I ate a <em>cheese</em> sandwich, which was nice.</p>', 'https://mastodon.example');

  t.is(result, 'I ate a cheese sandwich, which was nice.');
});

test('Convert HTML to status text, appending last link href if present', t => {
  const result = htmlToStatusText('<p>Hello <a href="/hello">world</a>, hello <a href="https://moon.example">moon</a>.</p>', 'https://mastodon.example');
