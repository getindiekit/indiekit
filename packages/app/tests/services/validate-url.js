...
test('Throws error if URL is not valid', async t => {
  const error = await t.throwsAsync(validateUrlService('foobar'));
...
test('Throws error if URL does not resolve', async t => {
  ...
  const error = await t.throwsAsync(validateUrlService(t.context.url));
...
test('Throws error if file at URL is not JSON', async t => {
  ...
  const error = await t.throwsAsync(validateUrlService(t.context.url, 'json'));
